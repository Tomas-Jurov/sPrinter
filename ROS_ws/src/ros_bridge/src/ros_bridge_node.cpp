...
int main(int argc, char** argv)
{
  ...
  ROSBridge ros_bridge(encoders_left_pub, encoders_right_pub, encoders_location_pub, imu_pub, 
											stepper1_current_pub, stepper2_current_pub, servo1_pub, servo2_pub, suntracker_fb_pub);

  ros::Subscriber left_speed_target_sub =
    nh.subscribe("wheels/cmd_vel", 1, &ROSBridge::wheelsSpeedTargetCallback, &ros_bridge);
  ros::Subscriber tilt_speed_target_sub =
		nh.subscribe("tilt/target_speed", 1, &ROSBridge::tiltSpeedTargetCallback, &ros_bridge);
